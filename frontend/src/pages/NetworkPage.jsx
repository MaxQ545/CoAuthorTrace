...
function NetworkPage() {
  ...
  const depthLegend = [
    { key: 'seed', label: t('network.legend_seed'), bg: '#2563EB', border: '#1E40AF' },
    { key: 'd1', label: t('network.legend_depth1'), bg: '#8B5CF6', border: '#6D28D9' },
    { key: 'd2', label: t('network.legend_depth2'), bg: '#EC4899', border: '#BE185D' },
    { key: 'd3', label: t('network.legend_depth3'), bg: '#F97316', border: '#C2410C' },
  ];

  useEffect(() => {
    document.title = t('network.page_title');
    return () => { document.title = t('network.default_title'); };
  }, [t]);
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm p-4 space-y-3">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <div className="flex items-start gap-2 shrink-0">
            ...
            >
              <Play size={14} />
              {isStreaming ? t('network.building') : t('network.build')}
            </button>
            ...
            {hasResults && (
              <>
                ...
                <button
                  ...
                  title={t('network.export_nodes_title')}
                >
                  <Download size={14} /> {t('network.export_nodes')}
                </button>
                <button
                  onClick={() => {
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2.5 bg-secondary text-secondary-foreground rounded-xl hover:bg-secondary/80 transition-colors text-sm"
                  title={t('network.export_edges_title')}
                >
                  <Download size={14} /> {t('network.export_edges')}
                </button>
              </>
            )}
              ...
              <button
                onClick={() => {
                  navigator.clipboard.writeText(window.location.href).then(() => {
                    toast.success(t('network.link_copied'));
                  }).catch(() => {
                    toast.error(t('network.copy_failed'));
                  });
                }}
                ...
                title={t('network.copy_share_link')}
              >
                <Share2 size={14} />
              </button>
            )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-3 border-t border-border">
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">
                    {t('network.depth')} (depth: {maxDepth})
                  </label>
                  <input
                    type="range"
                    min={1}
                  ...
                  <div className="flex justify-between text-[10px] text-muted-foreground mt-0.5">
                    <span>1</span>
                    <span>2</span>
                    <span>3 ({t('network.depth_recommended')})</span>
                    <span>4</span>
                    <span>5</span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex justify-between text-[10px] text-muted-foreground mt-0.5">
                    <span>5</span>
                    <span>20 ({t('network.depth_recommended')})</span>
                    <span>50</span>
                  </div>
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">
                    {t('network.max_nodes')} (max: {maxNodes})
                  </label>
                  <input
                    type="range"
                    min={100}
                  ...
                  <div className="flex justify-between text-[10px] text-muted-foreground mt-0.5">
                    <span>100</span>
                    <span>500 ({t('network.depth_recommended')})</span>
                    <span>2000</span>
                  </div>
                </div>
              </div>
        ...
        {(maxDepth * topK > 100 || maxNodes > 1000) && (
          <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 text-xs pt-2 border-t border-border">
            ...
            <span>{t('network.param_warning')}</span>
          </div>
        )}
      </div>
        ...
        <div className="lg:col-span-3 flex flex-col bg-card rounded-xl border border-border shadow-sm overflow-hidden relative">
          ...
          <div className={`flex-1 relative ${hasResults || isStreaming ? '' : 'invisible'}`}>
            ...
            {seedAuthors.length > 0 && hasResults && (
              <div className="absolute top-4 left-4 bg-background/90 backdrop-blur border border-border p-3 rounded-lg shadow-lg max-w-xs">
                <div className="text-sm font-semibold text-foreground flex items-center gap-2 mb-1">
                  ...
                  {t('network.multi_author')}
                </div>
                <div className="text-xs text-muted-foreground">
                  {seedAuthors.map(a => a.display_name).join(', ')}
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  {nodesCount} {t('network.nodes_count')} · {edgesCount} {t('network.edges_count')}
                </div>
              </div>
            )}
            ...
            {hasResults && (
              <div className="absolute bottom-4 right-4 bg-background/90 backdrop-blur border border-border p-3 rounded-lg shadow-lg text-xs space-y-1.5">
                <div className="font-medium text-foreground mb-1">{t('network.legend')}</div>
                ...
                <div className="flex items-center gap-2 pt-1 border-t border-border">
                  ...
                  <span className="text-muted-foreground">{t('network.collaboration')}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {!hasResults && !isStreaming && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-muted-foreground p-8 bg-card">
              ...
              <h3 className="text-lg font-medium text-foreground mb-2">{t('network.ready')}</h3>
              <p className="max-w-sm text-center text-sm">
                {t('network.ready_desc')}
              </p>
              <div className="mt-6 grid grid-cols-3 gap-4 text-center">
                <div className="space-y-1">
                  ...
                  <div className="text-xs">{t('network.select_authors')}</div>
                </div>
                <div className="space-y-1">
                  <div className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center mx-auto">
                    <Play size={16} className="text-primary" />
                  </div>
                  <div className="text-xs">{t('network.build')}</div>
                </div>
                <div className="space-y-1">
                  <div className="w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center mx-auto">
                    <Network size={16} className="text-primary" />
                  </div>
                  <div className="text-xs">{t('network.explore_relations')}</div>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-1 flex flex-col gap-4 overflow-hidden">
          {sidebarNodes.length > 0 ? (
            <div className="bg-card rounded-xl border border-border shadow-sm flex flex-col h-full overflow-hidden">
              <div className="p-4 border-b border-border bg-muted/30">
                <h3 className="font-semibold text-foreground flex items-center gap-2">
                  <Users size={16} />
                  {t('network.sidebar_nodes')} ({nodesCount})
                </h3>
              </div>
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm p-6 text-center h-full flex flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="text-sm">{t('network.sidebar_empty')}</p>
            </div>
          )}
        </div>
      </div>
